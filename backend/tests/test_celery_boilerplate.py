...
def test_celery_boilerplate_task_direct(function_mock, celery_app, celery_worker):
    """
...
@patch('app.tasks.debug.dummy_function')
def test_celery_boilerplate_send_task(function_mock, celery_app, celery_worker):
    """
    Shows how to test full celery loop - sending and executing task
    """
    function_mock.return_value = 44
    task_id = celery_app.send_task('tasks.dummy')
